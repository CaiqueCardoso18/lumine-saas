    ...
    case actionTypes.DISMISS_TOAST: {
      ...
      if (toastId) {
        if (!toastTimeouts.has(toastId)) {
          toastTimeouts.set(toastId, setTimeout(() => dispatch({ type: actionTypes.REMOVE_TOAST, toastId }), TOAST_REMOVE_DELAY));
        }
      } else {
        state.toasts.forEach((t) => {
          if (!toastTimeouts.has(t.id)) {
            toastTimeouts.set(t.id, setTimeout(() => dispatch({ type: actionTypes.REMOVE_TOAST, toastId: t.id }), TOAST_REMOVE_DELAY));
          }
        });
      }
